@echo off
chcp 65001 >nul
title Геркулес Клуб — оновлення
setlocal
cd /d "%~dp0"

echo.
echo   ГЕРКУЛЕС КЛУБ — оновлення застосунку
echo   -------------------------------------
echo.
echo   Перед оновленням закрийте вікна CRM ^(start-desk.bat^),
echo   щоб не працювати з файлами, поки вони змінюються.
echo.
pause

echo.
echo   [1/2] Отримання оновлень з GitHub...
git pull
if errorlevel 1 (
  echo.
